"""
Generate Gemini 3.5 Flash Evaluation Deck
=========================================
Creates a 3-slide format per conversation for all 5 cases using the gemini-3.5-flash model:
  ...
  3. Slide 3: SOAP report comparison table (Ground Truth vs Gemini 3.5 Flash Predicted)
"""
...
PREDICTED_DIR = BASE_DIR / 'predicted' / 'gemini-3.5-flash'
GROUND_TRUTH_DIR = BASE_DIR / 'ground_truth'

OUTPUT_FILE = BASE_DIR / 'Gemini_3.5_Flash_Detailed_Presentation.pptx'
...
def read_diarized_transcript(case_id):
    ...
    txt_path = PREDICTED_DIR / f'{case_id}_transcript.txt'
...
def build_title_slide(prs, case_id):
    slide = prs.slides.add_slide(prs.slide_layouts[6])
    set_slide_bg(slide, BG_DARK)
    add_text_box(slide, Inches(2), Inches(3), Inches(9.3), Inches(1),
                 f'Case Study: {case_id}',
                 font_size=40, color=ACCENT_BLUE, bold=True, alignment=PP_ALIGN.CENTER)
    add_text_box(slide, Inches(2), Inches(4), Inches(9.3), Inches(0.5),
                 f'Model Analysis using {MODEL_LABEL}',
                 font_size=20, color=TEXT_GRAY, alignment=PP_ALIGN.CENTER)

def build_dialogue_slide(prs, case_id, stt_text, gt_text):
    slide = prs.slides.add_slide(prs.slide_layouts[6])
    set_slide_bg(slide, BG_DARK)

    add_text_box(slide, Inches(0.5), Inches(0.3), Inches(12.3), Inches(0.6),
                 f'{case_id} — Speech-to-Text vs Real Transcript',
                 font_size=24, color=ACCENT_BLUE, bold=True)
    add_text_box(slide, Inches(0.5), Inches(0.85), Inches(12.3), Inches(0.35),
                 f'Comparison of Ground Truth dialogue vs {MODEL_LABEL} STT output (first ~100 words)',
    ...
    add_rounded_rect(slide, left_x + Inches(0.3), panel_top + Inches(0.2), Inches(3.5), Inches(0.45), ACCENT_GREEN)
    add_text_box(slide, left_x + Inches(0.3), panel_top + Inches(0.22), Inches(3.5), Inches(0.4),
                 '📄 Real Transcript (Ground Truth)', font_size=14, color=BG_DARK, bold=True, alignment=PP_ALIGN.CENTER)
    gt_100 = first_n_words(gt_text, 100)
    add_text_box(slide, left_x + Inches(0.3), panel_top + Inches(0.85), panel_w - Inches(0.6), panel_h - Inches(1.1),
                 gt_100, font_size=14, color=TEXT_WHITE)

    right_x = Inches(6.9)
    add_rounded_rect(slide, right_x, panel_top, panel_w, panel_h, BG_CARD, BORDER_COLOR)
    add_rounded_rect(slide, right_x + Inches(0.3), panel_top + Inches(0.2), Inches(3.8), Inches(0.45), ACCENT_ORANGE)
    add_text_box(slide, right_x + Inches(0.3), panel_top + Inches(0.22), Inches(3.8), Inches(0.4),
                 f'🎙️ Speech-to-Text Transcript ({MODEL_LABEL})', font_size=14, color=BG_DARK, bold=True, alignment=PP_ALIGN.CENTER)
    stt_100 = first_n_words(stt_text, 100)
    add_text_box(slide, right_x + Inches(0.3), panel_top + Inches(0.85), panel_w - Inches(0.6), panel_h - Inches(1.1),
                 stt_100, font_size=14, color=TEXT_WHITE)

def build_summary_slide(prs, case_id, pred_summary, gt_summary):
    slide = prs.slides.add_slide(prs.slide_layouts[6])
    set_slide_bg(slide, BG_DARK)

    add_text_box(slide, Inches(0.5), Inches(0.3), Inches(12.3), Inches(0.6),
                 f'{case_id} — Transcript Summary Comparison',
                 font_size=24, color=ACCENT_BLUE, bold=True)
    add_text_box(slide, Inches(0.5), Inches(0.85), Inches(12.3), Inches(0.35),
                 f'Clinical summary extracted from Ground Truth vs {MODEL_LABEL} STT output (first ~100 words)',
                 font_size=14, color=TEXT_GRAY)
    ...
    add_rounded_rect(slide, left_x, panel_top, panel_w, panel_h, BG_CARD, BORDER_COLOR)
    add_rounded_rect(slide, left_x + Inches(0.3), panel_top + Inches(0.2), Inches(4.0), Inches(0.45), ACCENT_GREEN)
    add_text_box(slide, left_x + Inches(0.3), panel_top + Inches(0.22), Inches(4.0), Inches(0.4),
                 '📄 Ground Truth Summary', font_size=14, color=BG_DARK, bold=True, alignment=PP_ALIGN.CENTER)
    ...
    right_x = Inches(6.9)
    add_rounded_rect(slide, right_x, panel_top, panel_w, panel_h, BG_CARD, BORDER_COLOR)
    add_rounded_rect(slide, right_x + Inches(0.3), panel_top + Inches(0.2), Inches(4.2), Inches(0.45), ACCENT_ORANGE)
    add_text_box(slide, right_x + Inches(0.3), panel_top + Inches(0.22), Inches(4.2), Inches(0.4),
                 f'🎙️ {MODEL_LABEL} Summary (STT)', font_size=14, color=BG_DARK, bold=True, alignment=PP_ALIGN.CENTER)
    pred_100 = first_n_words(pred_summary, 100)
    add_text_box(slide, right_x + Inches(0.3), panel_top + Inches(0.85), panel_w - Inches(0.6), panel_h - Inches(1.1),
                 pred_100, font_size=14, color=TEXT_WHITE)

def build_soap_table_slide(prs, case_id, pred_data, gt_data):
    slide = prs.slides.add_slide(prs.slide_layouts[6])
    set_slide_bg(slide, BG_DARK)
...
def main():
    ...
    print(f'Building Gemini 3.5 Flash Detailed Presentation...')

    for case_id in CASES:
        print(f"  Adding slides for {case_id}...")
        ...
        # We only generate slides if we actually have predicted data for this case
        if not pred_data:
            print(f"    Warning: No predicted data found for {case_id}. Skipping.")
            continue
            
        build_title_slide(prs, case_id)
        build_dialogue_slide(prs, case_id, stt_text, gt_text)
        build_summary_slide(prs, case_id, pred_summary, gt_summary)
        build_soap_table_slide(prs, case_id, pred_data, gt_data)
